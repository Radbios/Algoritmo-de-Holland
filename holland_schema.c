...
int checarCruzamento(int pai[], int mae[]){
    for(int i = 0; i < TAM_CROMO; i++){
        if(pai[i] != mae[i]){
            return 1;
        }
...
int schema(int pai[], int mae[], int option, int lista[]){
    int contador = 0;
    if(option == 1){
        for(int i = 0; i < TAM_CROMO; i++){
            if(pai[i] != mae[i]){
...
void cruzamento(int pop[][TAM_CROMO], int descPop[][TAM_CROMO], int *individuo, int cruzar){
    int lista[TAM_CROMO];
    for(int i = 0; i < cruzar - 1; i++){
        for(int j = i + 1; j < cruzar; j++){
            if(checarCruzamento(pop[i], pop[j]) ){
                int prob = (rand() % 100) + 1;

                if(prob > 60){
                   // int cortar = (rand() % TAM_CROMO);
                   int cortar = (rand() % schema(pop[i], pop[j], 1, lista));
                    for(int k = 0; k <= lista[cortar]; k++){
                        descPop[*individuo][k] = pop[i][k];
                        descPop[*individuo + 1][k] = pop[j][k];
                    }

                    for(int l = lista[cortar] + 1; l < TAM_CROMO; l++){
...
void mutacao(int pop[][TAM_CROMO], int descPop[][TAM_CROMO], int *individuo, int mutar){
    for (int i = mutar; i < 10; i++){
        int prob = (rand() % 100) + 1;

        if(prob > 90){
            int outroInd = (rand() % (10 - mutar)) + mutar;
            int lista[TAM_CROMO];
            int contador = rand() % schema(pop[i], pop[outroInd], 0, lista);
            printf("contador = %d\n",contador);
            for(int j = 0; j < contador; j++){
                ...
                if(alelo == 0){
                    ...
                    else descPop[*individuo][lista[j]] = 0; 
                }
                else descPop[*individuo][lista[j]] = pop[i][lista[j]];
            }
            *individuo += 1; 
        }
    }
...
int main(){
    ...
    while(adap[0] != TAM_CROMO/2){
        ...
        *individuo = 0;
        ...
        if(cruzar != -1){
            cruzamento(pop, descPop, individuo, cruzar);
        }

        if(mutar != -1){

            mutacao(pop, descPop, individuo, mutar);
